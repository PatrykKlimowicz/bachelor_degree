...
private:
    ...
    void setupRoutes() {
        ...
        Routes::Post(router, "/led/:value", Routes::bind(&StatsEndpoint::ledHandler, this));
        Routes::Get(router, "/random/:value/", Routes::bind(&StatsEndpoint::btnHandler, this));
    ...
    int ledHandler(const Rest::Request& request, Http::ResponseWriter response) {
        ...
        m_family_id = genl_ctrl_resolve(nl_sock, "led");
        if (m_family_id < 0) {
            nl_socket_free(nl_sock);
            return -1;
        }
    ...
    int btnHandler(const Rest::Request& request, Http::ResponseWriter response) {
        ...
        m_family_id = genl_ctrl_resolve(nl_sock, "btn");
        if (m_family_id < 0) {
            nl_socket_free(nl_sock);
            return -1;
        }
